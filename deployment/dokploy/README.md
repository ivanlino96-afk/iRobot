...
- mqtt.example.com: registro DNS A hacia la VPS. MQTT nativo TLS usa TCP 8883 directamente; no añadirlo como dominio HTTP de Dokploy. Si se utiliza Cloudflare, este registro debe estar en modo Solo DNS.
...
private/
  ...
  mqtt/server.crt                # certificado MQTT y cadena intermedia
  mqtt/server.key                # clave privada del certificado MQTT
  broker/dynamic-security.json   # base de credenciales de Mosquitto
```

El certificado debe incluir el nombre real MQTT_PUBLIC_HOST y ser confiable para el teléfono. Para producción, emitirlo con una CA pública mediante validación DNS del dominio MQTT y configurar su renovación. El HTTPS de Dokploy para la API no proporciona automáticamente el certificado del listener MQTT. No usar los certificados ficticios de firmware/test/build-config.

La API corre con UID 1000 y necesita leer `api/signing.pem`. Mosquitto corre con UID 1883 y necesita leer los archivos `mqtt/` y escribir en `broker/`. Usar directorios privados y permisos de lectura limitados, no chmod 777. La clave pública y la CA son los únicos archivos criptográficos que se copian al firmware; nunca la clave privada RSA ni las credenciales administrativas MQTT.
...
6. Permitir TCP 8883 en el firewall del proveedor para MQTT y comprobar que nadie lo esté utilizando. Mantener 80/443 según la instalación existente de Dokploy. No publicar 5432 ni 8080.
...
openssl s_client -connect mqtt.example.com:8883 -servername mqtt.example.com -verify_return_error </dev/null
```
